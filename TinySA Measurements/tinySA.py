...
path = fr'C:\Users\wonde\Documents\Extra_School_files\NSERC Research\TinySA_measurement_stuff\recorded_values_debug.csv'
...
#		print(line)
	...
	def data_measured(self, array = 2):
		...
		for line in data.split('\n'):
			if line:
				...
				#filtering for noise
				if("-:.0" in d[0]):
					d[0] = global_last_value[0]

				if(len(d) > 1):
					if("-:.0" in d[1]):
						d[1] = global_last_value[1]
				else:
					d.append(global_last_value[1])

				d=[float(d[0]), float(d[1])]
				x.append(d)
				global_last_value = d

		#writer.writerow([f"ended with array={array}"])		
		return x
	...
	def send_scan(self, start = 1e6, stop = 900e6, points = None):
		if points:
			self.send_command("scan %d %d %d %d\r"%(start, stop, points, 3))
		else:
	...
	def scan(self):
		...
		while len(freqs) > 0:
			...
			array0.extend(self.data_measured(2))
			freqs = freqs[segment_length:]
		self.resume()
		return array0
	
	def scan_range(self, start, stop):
		num_points = int(((stop-start)/15e6)*450)
		current_start = start
		values=[]
		while(num_points > 450):
			self.set_sweep(current_start, current_start+(15e6))
			self.fetch_frequencies()
			
			self.send_scan(self._frequencies[0], self._frequencies[-1], 450)
			values.extend(self.data_measured(2))
			num_points -= 450
			current_start += 15e6
			time.sleep(3)
		...
		self.send_scan(self._frequencies[0], self._frequencies[-1], 450)
		values.extend(self.data_measured(2)[:num_points])
		self.resume()
		return values
